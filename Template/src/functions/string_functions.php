...
class ezcTemplateStringFunctions extends ezcTemplateFunctions
{
    public static function getFunctionSubstitution( $functionName, $parameters )
    {
        switch( $functionName )
        {
            ...
            case "str_replace": 
                return array( array("%left", "%index", "%length", "%right"), 
                   self::concat( 
            ...
            // substr( $s, 0, $index ) . substr( $s, $index + $len );
             case "str_remove": 
                 return array( array("%string", "%index", "%length"), 
                   self::concat( 
            ...
            // substr( $s, 0, strlen( $string ) - $len );
             case "str_chop": 
                return array( array("%string", "%length"), 
            ...
            // substr( $s, $len );
             case "str_chop_front": return array( array("%string", "%length"), self::functionCall( "substr", array( "%string", "%length" ) ) );

             case "str_append": return array( array("%left", "%right"), self::concat( "%left", "%right" ) );

             case "str_prepend": return array( array("%left", "%right"), self::concat( "%right", "%left" ) );

            // str_compare( $sl, $sr )
            // strcmp( $sl, $sr );
            case "str_compare": return array( array( "%left", "%right"), self::functionCall( "strcmp", array( "%left", "%right" ) ) );

            // str_nat_compare( $sl, $sr )
            // strnatcmp( $sl, $sr );
            case "str_nat_compare": return array( array( "%left", "%right"), self::functionCall( "strnatcmp", array( "%left", "%right" ) ) );

            //str_contains( $sl, $sr ) ( QString::compare )::
            // strpos( $sl, $sr ) !== false 
            case "str_contains": return array( array( "%left", "%right"), 
                array( "ezcTemplateNotIdenticalOperatorAstNode", 
                array( self::functionCall( "strpos", array( "%left", "%right" ) ), self::value( false ) ) ) );

            // str_len( $s )
            // strlen( $s )
            case "str_len": return array( array("%string"), self::functionCall( "strlen", array( "%string" ) ) );

            // str_left( $s, $len )
            // substr( $s, 0, $len )
            case "str_left": return array( array("%string", "%length"), self::functionCall( "substr", array( "%string", self::value(0), "%length" ) ) );
            ...
            // str_right( $s, $len )
            // substr( $s, -$len )
            case "str_right": return array( array("%string", "%length"), 
            ...
            // str_mid( $s, $index, $len )
            // substr( $s, $index, $len )
            case "str_mid": return array( array("%string", "%index", "%length"), 
                self::functionCall( "substr", array( "%string", "%index", "%length") ) );

            // str_at( $s, $index )
            // substr( $s, $index, 1 )
            case "str_at": return array( array("%string", "%index"), 
                self::functionCall( "substr", array( "%string", "%index", self::value(1) ) ) );

            // str_fill( $s, $len )
            // str_repeat( $s, $len )
            case "str_fill": return array( array("%string", "%length"), 
                self::functionCall( "str_repeat", array( "%string", "%length" ) ) );

            // str_index_of( $sl, $sr [, $index ] )
            // strpos( $sl, $sr [, $index ] )
            case "str_index_of": return array( array("%haystack", "%needle", "[%index]"), 
            ...
            // str_is_empty( $s )
            // strlen( $s ) === 0
            case "str_is_empty": return array( array("%string"), 
                array( "ezcTemplateIdenticalOperatorAstNode", array( 
            ...
            case "str_pad_left": return array( array("%string", "%length", "%fill"), 
            ...
            case "str_pad_right": return array( array("%string", "%length", "%fill"), 
            ...
            // str_split( $s, $sep[, $max] )
            // explode( $s, $sep, $max )
            case "str_split": return array( array("%string", "%separator", "[%max]"), 
                    self::functionCall( "explode", array( "%separator", "%string", "[%max]") ) );
             
            // str_join( $s_list, $sep )
            // join( $sList, $sep )
            case "str_join": return array( array("%list", "%separator"), 
            ...
            // str_ord( $c )
            // chr( $c )
            case "str_ord": return array( array("%char"), 
            ...
            // TODO
             ...
             // str_char_count( $s )::
            // strlen( $s )
            case "str_char_count": return array( array( "%string" ), 
                    self::functionCall( "strlen", array( "%string" ) ) );
             
            // str_word_count( $s [, $wordsep] )
            // str_word_count( $s, 0 [, $wordsep] )
            case "str_word_count": return array( array( "%string", "[%wordsep]" ), 
                    self::functionCall( "str_word_count", array( "%string", self::value(0), "[%wordsep]" ) ) );
 
            // - *string* str_paragraph_count( $s )::
            // Code.
            case "str_paragraph_count": return array( array( "%string" ), 
